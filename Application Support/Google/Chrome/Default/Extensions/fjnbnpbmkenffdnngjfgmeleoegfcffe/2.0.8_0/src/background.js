...
const optOutAndOpenWelcome = async (shouldOptout, type) => {
    return new Promise((resolve, reject) => {
...
chrome.runtime.onInstalled.addListener(async (details) => {
    ...
    if (details.reason === "install") {
        ...
        if (browserToUse === 'firefox') {
            await optOutAndOpenWelcome(true, details.reason);
        } else {
        ...
        //optout firefox users who updated the extension and show them the welcome page if they have never seen it
        //todo: remove before next FF version
        if (browserToUse === 'firefox') {
            await optOutAndOpenWelcome(true, details.reason);
        }
